...
import explainPythonCodeSkill, {
    PROJECT_ROOT,
    resolveSafeFilePath,
} from "../skills/explainPythonCodeSkill.js"
...
test("rejects an unsafe file path before touching the file tool", async () => {
    ...
    const result = await explainPythonCodeSkill.execute({
        filePath: "../../etc/passwd",
        toolBus: { execute: async () => ({ success: true, content: "" }) },
        explainPythonCode: async () => ({ explanation: "should not run" }),
    })
...
test("happy path: reads via the file tool and returns the explanation", async () => {
