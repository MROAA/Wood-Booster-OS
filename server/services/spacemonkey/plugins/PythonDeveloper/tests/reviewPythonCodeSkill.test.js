...
import reviewPythonCodeSkill, {
    PROJECT_ROOT,
    resolveSafeFilePath,
} from "../skills/reviewPythonCodeSkill.js"
...
test("rejects an unsafe file path before touching the file tool", async () => {
    ...
    const result = await reviewPythonCodeSkill.execute({
        filePath: "../../etc/passwd",
        toolBus: { execute: async () => ({ success: true, content: "" }) },
        reviewPythonCode: async () => ({ review: "should not run" }),
    })
...
test("happy path: reads via the file tool and returns the review", async () => {
    ...
    assert.equal(result.review, "reviewed: print('hello')")
