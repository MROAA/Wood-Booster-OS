...
// economy too - I'll use it myself to adjust the game's difficulty).
// These 10 levers used to be declared right here in runEngine.js
// (HIGH risk - Hearthwood Studio only live-applies plain data files
// under src/data/heartwood/) - moved to economyLevers.js, pure data.
// The 8 scalars live there as ONE object map (ECONOMY_LEVERS) - the
// Studio's entity reader only walks a single object-map/array export
// per registered type, not several loose top-level consts - so they're
// destructured back into their old local names here, changing nothing
// about how the rest of this file reads them. SHOP_INVESTMENTS and
// MARKET_EVENTS were already proper object-map exports, so they moved
// unchanged. Every one of the previously-public six is re-exported
// below so every OTHER file that already imported it directly from
// runEngine.js (SquadDraft.jsx, HeartwoodBattle.jsx) keeps working
// unchanged - the other four (WIN_ESSENCE and the three per-fight-type
// bonuses) were module-private before and stay that way.
const {
  winEssence: WIN_ESSENCE,
  formationBonusEssence: FORMATION_BONUS_ESSENCE,
  minibossBonusEssence: MINIBOSS_BONUS_ESSENCE,
  eliteBonusEssence: ELITE_BONUS_ESSENCE,
  gambleCost: GAMBLE_COST,
  interestRate: INTEREST_RATE,
  interestThreshold: INTEREST_THRESHOLD,
  interestCap: INTEREST_CAP,
} = ECONOMY_LEVERS
export { GAMBLE_COST, INTEREST_RATE, INTEREST_THRESHOLD, INTEREST_CAP, SHOP_INVESTMENTS, MARKET_EVENTS }
