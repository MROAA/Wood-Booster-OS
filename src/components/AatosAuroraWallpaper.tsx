import React, { useState } from 'react';

export const SpacemonkeyDefrag: React.FC = () => {
  const [defragStatus, setDefragStatus] = useState<string>('Memory Heap Optimal');
  const [progress, setProgress] = useState<number>(100);

  const runDefrag = () => {
    setDefragStatus('Aligning Spacemonkey Neural Sectors...');
    setProgress(45);
    setTimeout(() => {
      setProgress(100);
      setDefragStatus('Defrag Complete: 0% Fragmentation');
    }, 3000);
  };

  return (
    <div className="absolute bottom-4 left-4 bg-[#c0c0c0] border-2 border-t-white border-l-white border-b-black border-r-black p-2 w-72 shadow-xl font-sans text-xs text-black z-40 select-none">
      <div className="bg-[#000080] text-white px-2 py-1 font-bold flex justify-between items-center mb-2 border border-black cursor-move">
        <span>Spacemonkey_Defrag.exe</span>
        <span className="flex gap-1">
          <button className="bg-[#c0c0c0] text-black px-1 border border-black text-[10px]">_</button>
          <button className="bg-[#c0c0c0] text-black px-1 border border-black text-[10px]">X</button>
        </span>
      </div>
      <div className="space-y-1 bg-white p-2 border-2 border-t-black border-l-black border-b-white border-r-white">
        <div className="flex justify-between">
          <span className="font-bold">Status:</span>
          <span className="text-blue-900 font-bold">{defragStatus}</span>
        </div>
        <div className="flex justify-between">
          <span className="font-bold">Heap Integrity:</span>
          <span className="text-green-700 font-mono font-bold">{progress}%</span>
        </div>
        <div className="w-full bg-gray-300 h-2 mt-1 border border-black">
          <div className="bg-blue-800 h-full transition-all duration-500" style={{ width: `${progress}%` }}></div>
        </div>
      </div>
      <div className="mt-2">
        <button 
          onClick={runDefrag}
          className="w-full bg-[#c0c0c0] border-2 border-t-white border-l-white border-b-black border-r-black px-2 py-1 font-bold active:border-t-black active:border-l-black active:border-b-white active:border-r-white hover:bg-[#d0d0d0]"
        >
          Run Quantum Defrag
        </button>
      </div>
    </div>
  );
};
