...
/*
 * History: GET /api/bloodmoor-patchbay (?archived=) - newest first
 * (auditStore.list). `reloadKey` changing (parent bumps it after every
 * successful apply()) triggers a refetch.
 */
function PatchHistoryList({ reloadKey, onReverted }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        const data = await apiGet("/bloodmoor-patchbay?archived=false")
  ...
  async function handleRevert(id) {
    if (!window.confirm("Revert this change and restore the previous value?")) {
      return
    }
    ...
    try {
      await apiPost(`/bloodmoor-patchbay/${id}/revert`, {})

      const refreshed = await apiGet("/bloodmoor-patchbay?archived=false")
