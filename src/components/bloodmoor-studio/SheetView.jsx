...
/*
 * A spreadsheet-style grid: every entity of the selected type as a row,
 * every scalar field as a column, edit as many cells as you like, then
 * ONE batched preview/apply for the whole set. Ported from
 * hearthwood-studio/SheetView.jsx - fully generic (keyed off whatever
 * fields the API reports), works unchanged for both Bloodmoor's "map"
 * type (weapons, several rows) and "single" types (one row - e.g.
 * boss/spawner), no Bloodmoor-specific logic needed here at all.
 */
...
function SheetView({ type, onApplied }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        const data = await apiGet(`/bloodmoor-patchbay/entities?type=${type}`)
  ...
  const columns = useMemo(() => {
    ...
    ordered.sort((a, b) => (a === "label" ? -1 : b === "label" ? 1 : 0))
  ...
  const dirtyCount = Object.values(edits).reduce((total, fields) => total + Object.keys(fields).length, 0)
  ...
  async function handlePreview() {
    ...
    for (const entity of entities) {
      ...
      for (const [field, raw] of Object.entries(entityEdits)) {
        ...
        ops.push({ path: [entity.id, field], op: "set", value: coerceValue(cellField.kind, raw) })
      }
    }
...

      {
        result && (
          <div className="shrink-0 p-4">
            <PatchPreviewPanel result={result} onDiscard={discard} onApply={apply} applying={applying} />
          </div>
        )
      }
