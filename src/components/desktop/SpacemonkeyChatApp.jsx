...
const SPACEMONKEY_URL = 'http://localhost:8002/api/spacemonkey/chat';

export default function SpacemonkeyChatApp() {
  const [messages, setMessages] = useState([
    { sender: 'spacemonkey', text: 'Moi. Kysy mitä vain - vastaan oikean Spacemonkey-ytimen ja tekoälyn kautta.' },
  ]);
      ...
      <div className="chat-messages" ref={listRef}>
        {messages.map((m, i) => (
          <div key={i} className={`chat-row ${m.sender === 'user' ? 'user' : 'spacemonkey'}`}>
            ...
            <div className={`chat-bubble${m.blocked ? ' chat-bubble-blocked' : ''}`}>{m.text}</div>
          </div>
        ))}
        {sending && (
