...
export default function RelicChoice({ runState, onChoose, onReroll }) {
  // Stage B, screen 2 - re-checked live, same as EventScreen.jsx: this
  // screen's root is plain .hw-intro (no dedicated class of its own,
  // unlike .hw-event) - a plain block, no flex/grid/gap - the standard
  // wrapper-div pattern is safe, no scoped CSS override needed. The
  // relic grid itself IS a real CSS Grid (.hw-select-grid), but it's
  // wrapped as ONE opaque block here (not split into individual card
  // slots) - Stage B's own scope is screen SECTIONS, matching Stage
  // A's granularity, not the Market's later per-card round.
  const layout = useFreeLayout({ screenId: "relicChoice", keys: ["header", "flavor", "relicGrid", "actions"] })
