  ...
  function simulateRun(characterId) {
    ...
    while (run.phase !== "victory" && run.phase !== "defeat" && safety < 400) {
      ...
      if (run.phase === "shop") {
        ...
        // feat/hearthwood-market-tiers: guarded so an older build (no
        // such export) skips it; the AFTER bot advances Tier + fields
        // the specialist units.
        if (engine.advanceMarketTier && engine.marketTierCost) {
          const tierCost = engine.marketTierCost(run.marketTier || 1)
          if (tierCost !== null && run.essence >= tierCost + 2) run = engine.advanceMarketTier(run)
        }
