...
const PORT = 5310
const TARGET_FIGHTS = 14 // lands on "embers-bulwark" - a shielding formation w/ AoE + self-heal, real status stacking potential
...
for (let fight = 0; fight < TARGET_FIGHTS; fight++) {
  ...
  if (fight < TARGET_FIGHTS - 1) {
    ...
    // Burst-capture several moments of the final fight instead of one
    // blind sleep, so at least one frame catches a real busy mid-fight
    // state even if this particular fight resolves quickly.
    for (let i = 0; i < 6; i++) {
      await page.waitForTimeout(500)
      await page.screenshot({ path: `.scratch/shots/06-busy-${i}.png`, fullPage: true })
      const overlayVisible = await page.locator(".hw-overlay").isVisible({ timeout: 200 }).catch(() => false)
